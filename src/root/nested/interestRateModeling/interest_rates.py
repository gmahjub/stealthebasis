...
class InterestRates(object):
    ...
    def calc_terminal_value_of_investment(self):
        return self.amountA * pow((1 + (self.ratePerAnnumR / self.compoundFreqPerAnnumM)),
                                  self.compoundFreqPerAnnumM * self.numYearsInvestedN)
...
class InterestRateInstrument(object):
    ...
    def get_yield_to_maturity(self,
        ...
        ytm_func = lambda y: sum([coupon / (1 + y / freq) ** (freq * t) for t in dt]) + par / (1 + y / freq) ** (
                    freq * self.get_term()) - price
    ...
    def get_price_from_ytm(self,
        ...
        par = self.get_par_value()
        dt = [(i + 1) / freq for i in range(int(periods))]
        price = sum([coupon / (1 + ytm / freq) ** (freq * t) for t in dt]) + par / (1 + ytm / freq) ** (
                    freq * self.get_term())
        return price
...
class YieldCurve(object):
    ...
    def zero_coupon_spot_rate(self,
        ...
        self.logger.info('interest_rates.YieldCurve.zero_coupon_spot_rate(): ParValue %s Price %s Term %s',
                         str(iri.get_par_value()), str(iri.get_price()), str(iri.get_term()))
    ...
    def get_spot_curve_from_par_curve(self):
        ...
        for i in range(0, len(iri_obj_list)):
            ...
            logger.info("interest_rates.YieldCurve.get_spot_curve_from_par_curve(): value %s %s %s %s ", str(value),
                        str(bey_i), str(t_i), str(summer))
            s.append(value)
...
class PiecewiseCubicSpline(object):
    ...
    def __init__(self,
        ...
        self.clean_yield_curve_tau_list = list(
            filter(lambda k: not np.isnan(self.yield_curve_dict[k].get_obs_ytm()), self.yield_curve_dict))
        self.clean_yield_curve_yield_list = [self.yield_curve_dict[clean_key].get_obs_ytm() for clean_key in
                                             sorted(set(self.clean_yield_curve_tau_list))]
...
class NelsonSiegel(object):
    ...
    def nelson_siegel(self,
        ...
        ns = self.beta1 + (self.beta2 + self.beta3) * (self.gamma_ns / tau) * (
                    1 - np.exp(-tau / self.gamma_ns)) - self.beta3 * np.exp(-tau / self.gamma_ns)
        return ns
    ...
    def nelson_siegel_svensson(self,
        ...
        nss = ns + self.beta4 * (self.gamma_nss / tau) * (1 - np.exp(-tau / self.gamma_nss)) - self.beta4 * np.exp(
            -tau / self.gamma_nss)
        return nss
    ...
    def calc_fmin(self,
        ...
        pf_ns = lambda bg, x: bg[0] + (bg[1] + bg[2]) * (gamma_ns / x) * (1 - np.exp(-x / gamma_ns)) - bg[2] * np.exp(
            -x / gamma_ns)
        pf_nss = lambda bg, x: bg[0] + (bg[1] + bg[2]) * (gamma_ns / x) * (1 - np.exp(-x / gamma_ns)) - bg[2] * np.exp(
            -x / gamma_ns) + bg[3] * (gamma_s / x) * (1 - np.exp(-x / gamma_s)) - bg[3] * np.exp(-x / gamma_s)
        ...
        x = np.array(
            clean_yield_curve_tau_list)  # these are the maturities, they should be in years, so 1 month would be 1/12 or .083
        y = np.array(
            clean_yield_curve_yield_list)  # these are the actual yields that correspond for the above maturities
        ...
        if method == 'NS':
            ...
            if minimizer == 'optimize.fmin':
                ...
                p = optimize.minimize(error_func_nss, p0_nss, args=(x, y), method='SLSQP', bounds=bnds,
                                      constraints=cons)
            elif minimizer == 'optimize.minimize.method=Nelder-Mead':
...
class HeathJarrowMortonModel(object):
    ...
    def calc_fmin_stats(self):
        ...
        self.logger.info('Nelder-Mead MFE-Exceed avg min func %s',
                         str(self.Nelder_Mead_mfe_exceed_min_func_summer / self.Nelder_Mead_mfe_exceed_counter))
    ...
    def NS_S_interpolation(self,
        ...
        for gamma_ns in np.linspace(0.25, 4.0, 16):
            for gamma_s in np.linspace(4, 0.25, 16):
                if (abs(gamma_ns - gamma_s) < 1.0):
                    ...
                    returned_min_params_SLSQP = nss.calc_fmin(gamma_ns, gamma_s,
                                                              minimizer='optimize.minimize.method=SLSQP')
        ...
        self.logger.info(
            logger_loc + ' nss.beta1, bss.beta2, nss.beta3, nss.beta4, gamma_ns, gamma_s %s, %s, %s, %s, %s, %s %s',
            str(date_of_yield_curve), \
        ...
        return_series = pd.Series(
            data=[nss.beta1, nss.beta2, nss.beta3, nss.beta4, nss.gamma_ns, nss.gamma_nss, nss.beta1 + nss.beta2,
                  abs(nss.gamma_ns - nss.gamma_nss), \
                  lowest_min_func_return_dict['yield_curve']], name=date_of_yield_curve, \
            index=['Beta1', 'Beta2', 'Beta3', 'Beta4', 'Gamma1', 'Gamma2', 'Beta1_Plus_Beta2',
                   'Abs_Gamma1_Minus_Gamma2', 'Yield_Curve'])
...
def do_NS_S_interpolation(row, row_counter, min_func_summer):
    ...
    clean_yield_curve_tau_list = list(
        filter(lambda k: not np.isnan(yield_curve_dict[k].get_obs_ytm()), yield_curve_dict))
    clean_yield_curve_yield_list = [yield_curve_dict[clean_key].get_obs_ytm() for clean_key in
                                    sorted(set(clean_yield_curve_tau_list))]
