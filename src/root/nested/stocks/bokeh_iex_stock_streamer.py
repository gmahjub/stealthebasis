...
This starts the bokeh server, and provides you with url.
'''
...
def get_last_price(symbol):
    # endpoint = "tops/last"
    # return pyex.lastDF(symbol)
    #print ("output of lastDF()...", pyex.lastDF('SPY,'))
    return pd.DataFrame([pyex.delayedQuote(symbol)])

def get_last_rt_price(symbol):
    return pd.DataFrame(pyex.last(symbol + ','))
...
def update_rt_price():
    new_price = get_last_rt_price(symbol=TICKER)
    data.stream(dict(time=new_price["time"],
                     display_time=new_price["time"],
...
curdoc().add_periodic_callback(update_rt_price, 1000)