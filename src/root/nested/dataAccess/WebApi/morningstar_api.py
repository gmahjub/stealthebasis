...
import pandas
# Reference - https://www.clips.uantwerpen.be/pages/pattern
...
class MS_StatsExtract(object):
    ...
    def __init__(self):
        ...
        self.ms_stats_extract_temp_csv = r'/Users/traderghazy/workspace/data/morningstar/ms_stats.csv'
        self.ms_stats_extract_temp_csv_transpose = r'/Users/traderghazy/workspace/data/morningstar/ms_stats_t.csv'
        ...
        def process_dataset(self):
            ...
            try:
                ...
                self.target_stock_data_df = pandas.read_csv(self.ms_stats_extract_temp_csv,
                                                            header=2,
                                                            thousands=',',
                                                            index_col=0,
                                                            skiprows=[19, 20, 31, 41, 42, 43, 48, 58, 53, 64, 65, 72,
                                                                      73, 95, 101, 102])
            except:
            ...
            # rename columns
            self.target_stock_data_df.rename(
                columns={'Year over Year': 'Revenue yoy', '3-Year Average': 'Revenue 3yr avg',
                         ...
                         '5-Year Average.1': 'Operating income 5yr avg',
                         '10-Year Average.1': 'Operating income 10yr avg',
            ...
            if len(self.com_data_allstock_df) == 0:
                ...
                self.com_data_allstock_df = pandas.concat([self.com_data_allstock_df, self.target_stock_data_df],
                                                          ignore_index=True)
        ...
        def add_parameters_for_target_stock_df(self):
            ...
            """
            self.target_stock_data_df['Earnings Per Share cal'] = self.target_stock_data_df['Net Income Mil'] / \
                                                                  self.target_stock_data_df['Shares Mil']
            self.target_stock_data_df['Dividends cal'] = self.target_stock_data_df['Earnings Per Share cal'] * \
                                                         self.target_stock_data_df['Payout Ratio %']
            self.target_stock_data_df['Return on Equity'] = self.target_stock_data_df['Net Income Mil'] / \
                                                            self.target_stock_data_df["Total Stockholders' Equity"]
        ...
        def get_trend_data(self):
            ...
            for label in ['EPS yoy', 'Revenue yoy', 'Net income yoy']:
                ...
                accel_growth_check = accel_growth_check.to_frame().rename(
                    columns={label: label + ' 4yr_accel'}).reset_index()
                normal_growth_check = normal_growth_check.to_frame().rename(
                    columns={label: label + ' 4yr_grow'}).reset_index()
                ...
                if len(self.hist_company_data_trends_df) == 0:
                    ...
                    self.hist_company_data_trends_df = pandas.merge(self.hist_company_data_trends_df, both_check_df,
                                                                    on='SYMBOL')
...
class MS_ValuationExtract(object):
    ...
    def url_site_download(self):
        ...
        try:
            ...
            if self.__print_download_fault:
                print ('Problem with processing this data: ', self.target_full_url)
            self.download_fault = 1
            return None
    ...
    def parse_valuation_data(self, url_html):
        ...
        rename_col_list = ['wp_history_' + str(year) for year in range(2005, 2015)] + ['wp_history_TTM'] + [
            'Valuation_type']
        rename_col_dict = {}
...
if __name__ == '__main__':
    ...
    if choice == 1:
        ...
        combined_df_2.to_csv(r'/Users/traderghazy/workspace/data/morningstar/morn_star_data.csv', index = False)
    ...
    if choice ==2:
        ...
        pp.get_com_data_fr_all_stocks()
        pp.com_data_allstock_df.to_csv(r'/Users/traderghazy/workspace/data/morningstar/morn_star_data.csv', index = False)
        #pp.get_trend_data()
    ...
    if choice ==3:
        ...
        yy.process_all_stock_data()
        yy.combined_valuation_df.to_csv(r'/Users/traderghazy/workspace/data/morningstar/morn_star_data1.csv', index = False)
