...
from root.nested.timeSeriesAnalysis.extendARIMA.arima_module import ArimaObject
from root.nested.timeSeriesAnalysis.extendARIMA.arima_module import ArimaOrder
## put this in to ignore warnings in evaluation of ARIMA
import warnings
from statsmodels.graphics.tsaplots import plot_acf, plot_pacf
from math import sqrt
from root.nested.timeSeriesAnalysis.stationary import StationaryObj
...
def plot_acf_pacf_ibm_ds_code(timeseries,
                              ...
                              plot_show=False):
    # Plot ACF:
    lag_acf, confint_acf, qstat, pvalues = acf(timeseries, qstat=True, nlags=10, alpha=(1 - ci),
                                               fft=b_fft)  # there are other arguements, document them.\
...
def do_ARIMA(weekly_boe_xudlerd_log_diff,
    ...
    pyplot.plot(weekly_boe_xudlerd_log_diff[arima_D_value:].index.to_pydatetime(), results_ARIMA.fittedvalues,
                color='red')
    ...
    rss = sum((results_ARIMA.fittedvalues - weekly_boe_xudlerd_log_diff.iloc[arima_D_value:,
                                            0]) ** 2)  # residual sum of squares
...
def do_in_sample_predict(results_ARIMA,
    ...
    print("WTF", wbxl.iloc[0])
    print(wbxl.columns)
    predictions_ARIMA_log = pd.Series(wbxl.iloc[0], index=weekly_boe_xudlerd_log_diff.index)
    print("predictions_ARIMA_log", predictions_ARIMA_log.tail(10))
...
def plot_in_sample_prediction(predictions_arima,
    ...
    pyplot.plot(weekly_boe_xudlerd_data.index[differencing_interval:].to_pydatetime(), predictions_arima.values,
                color='red')
    # they will always differ by value of 1
    rmse = np.sqrt(sum((predictions_arima - weekly_boe_xudlerd_data.iloc[differencing_interval:, 0]) ** 2) / len(
        weekly_boe_xudlerd_data))
    pyplot.title('RMSE: %.4f' % rmse)
    pyplot.show()
...
def evaluate_arima_model(timeseries_values,
    ...
    for t in range(len(test_values)):
        ...
        try:
            ...
            break
        yhat, err95, ci95 = model_fit.forecast(steps=1)
        forecast = model_fit.forecast(steps=1)[0]
        days = 1
        for fc in forecast:
            # we have to invert the difference here, using same interval as when we difference for stationarity
            yhat = reverse_difference_decomposition(history, fc, interval=differencing_interval)
...
def plot_stationarity_test(timeseries,
    ...
    _ = pyplot.plot(rolling_std_dev.index.to_pydatetime(), rolling_std_dev.values, color='black',
...
boe_xulerd_data = get_quandl_data("FOREX_TO_USD",
                                  'EUR_USD_spot',
                                  '.csv')
pyplot.figure()
# plot the raw data
...
s_obj = test_stationarity(weekly_boe_xudlerd_log_diff, 260, 'AIC',
                          title='Log First-Order Difference - Rolling Mean & Std Dev')
if s_obj.df_test_exception != None:
    print("Dickey Fuller DataCamp failed with exception ", s_obj.df_test_exception)
else:
    print("Dickey Fuller Stats ", s_obj.df_output)
...
for arima_object in list_of_ARIMA_objects:
    ...
    if results_ARIMA == -1:
        ...
        try:
            ...
            print("Reducing the MA term (Q value) by 1 to check for convergence...(", is_arima_order_obj.get_p_value(),
                  is_arima_order_obj.get_d_value(), is_arima_order_obj.get_q_value() - 1, ")")
    ...
    if out_sample_rmse == -1:
        ...
        try:
            ...
            os_arima_order_obj = ArimaOrder(new_p_value,
                                            new_d_value,
                                            new_q_value)
            print("Reducing the MA term (Q value) by 1 to check for convergence...(",
