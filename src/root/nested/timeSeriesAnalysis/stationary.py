...
class StationaryObj(object):
    ...
    def dickey_fuller_test(self,
        ...
        try:
            ...
            return False
        df_output = pd.Series(df_test[0:4], index=['Test Statistic', 'p_-value', '#Lags Used', 'Number of Observations Used'])
        is_stationary = False
    ...
    def calc_and_verify_MSE(self,
        ...
        if (calculated_mse != None and int(sklearn_mse*1000000.0) == int(calculated_mse*1000000.0)):
    ...
    def classical_seasonal_decomposition(self,
                                         model='additive', # model='multiplicative' is the other option, additive is default
        ...
        self.logger.info("stationarity.classical_seasonal_decomposition: DataFrame frequency: %s", str(self.timeseries.index.freq))
        classical_decomp_result_obj = seasonal_decompose(self.timeseries, model, filt=filter_type, two_sided=two_sided_bool )
    ...
    def loess_seassonal_decomposition(self,
                                      period_size,
                                      resample_freq='D',
                                      interpolation='linear'):  # period size is in interms of number of indexes since this is a DataFrame
